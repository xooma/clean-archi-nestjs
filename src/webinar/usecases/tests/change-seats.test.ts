...
describe('Feature: changing the number of seats', () => {
  ...
  describe('Scenario: updating the webinar of someone else', () => {
    ...
    it('should fail', async () => {
      await expect(useCase.execute(payload)).rejects.toThrow(
        new WebinarUpdateForbiddenException(),
      );
  ...
  describe('Scenario: reducing the number of seats', () => {
    ...
    it('should fail', async () => {
      await expect(useCase.execute(payload)).rejects.toThrow(
        new DomainException('You cannot the reduce the number of seats'),
      );
  ...
  describe('Scenario: reducing the number of seats', () => {
    ...
    it('should fail', async () => {
      await expect(useCase.execute(payload)).rejects.toThrow(
        new DomainException('The webinar must have a maximum of 1000 seats'),
      );
